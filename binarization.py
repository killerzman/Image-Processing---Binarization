...
class BinaryApp(wx.App):
	...
	def createWidgets(self):
		...
		#create the adaptive threshold button
		self.thresholdAdaptiveBtn = wx.Button(self.panel, label = "Adaptive Threshold")
		self.thresholdAdaptiveBtn.Bind(wx.EVT_BUTTON, self.onAdaptive)
		self.thresholdAdaptiveBtn.SetToolTip(wx.ToolTip("Calculate an adaptive threshold using a clustering algorithm (Iso Data Algorithm)"))
		self.thresholdAdaptiveBtn.Enable(False)
		...
		self.binaryImgSizer.Add(self.thresholdAdaptiveBtn, 0, wx.ALL, 5)
	...
	def onBrowse(self, event):
		...
		if dialog.ShowModal() == wx.ID_OK:
			...
			self.onImageLoad()
			self.thresholdAdaptiveBtn.Enable(True)
		dialog.Destroy()

	#when the adaptive threshold button is pressed
	def onAdaptive(self, event):
		self.hist = self.calcHist(self.grayImg)
		self.thresholdCurrentValue = self.isoDataAlgo()
		self.thresholdSlider.SetValue(self.thresholdCurrentValue)
	...
	#see comment for isoDataAlgo function
	def median(self, start, stop):
		p = 0
		x = 0
		...
		if p == 0:
			return stop if start == 0 else start

		return x/p

	def calcHist(self, wxImageObject):
		#numpy array with 256 zeros
		hist = np.zeros(256, dtype = 'uint')

		#get image data
		data = np.frombuffer(wxImageObject.GetData(), dtype = 'uint8')

		#there are 3 consecutive equal values everywhere in the image data
		#splicing it every 3rd value
		for value in data[0::3]:
			hist[value] += 1
		return hist

	#I have no idea how it works but it's some math magic :)
	def isoDataAlgo(self, t = None):
		if t is None:
			t = self.thresholdDefaultValue
		median1 = self.median(0, t)
		median2 = self.median(t + 1, 255)

		adaptiveThresh = (median1 + median2) / 2

		if t == adaptiveThresh:
			return int(adaptiveThresh)
		else:
			return self.isoDataAlgo(adaptiveThresh)
